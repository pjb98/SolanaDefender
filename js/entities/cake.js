...
function Corn(game, spawn){
	...
	this.game = game;
	Phaser.Sprite.call(this, this.game, spawn.x, spawn.y, 'cake');
...
Corn.prototype.update = function(){
	...
	if(this._state == this.STATES.TRACKING){
		...
		if(this.target){
			...
			else if(this.withinFollowingRange(this.target) || this.target.name == "solana"){ 
...
Corn.prototype.die = function(points){
	...
	e.makeParticles('cake_die', [0,1,2]);
	//e.gravity = 0;
	e.minRotation = 0;
	e.maxRotation = 0;
	...
	if(points){
		InventoryManager.points += MainGame.points.kill_cake;
	}
