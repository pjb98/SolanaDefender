...
function Bear(game, spawn){
	...
	this.game = game;
	Phaser.Sprite.call(this, this.game, spawn.x, spawn.y, 'popcat');
...
Bear.prototype.update = function(){
	...
	if(this._state == this.STATES.TRACKING){
		if(this.target){
			if(this.withinFollowingRange(this.target) || this.target.name == "solana"){ 
...
Bear.prototype.die = function(points){
	...
	e.makeParticles('popcat_die', [0,1,2,3]);
	//e.gravity = 10;
	e.maxRotation = 0;
	e.minRotation = 0;
	e.start(true, 400, null, 16);

	//var t = new Explosion(this.game, {x:this.x, y:this.y}, 'popcat_explode');
	...
	if(points){
		InventoryManager.points += MainGame.points.kill_popcat;
	}
	this.destroy();
}